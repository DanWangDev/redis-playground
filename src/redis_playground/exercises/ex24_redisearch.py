...
class Ex24RediSearch(ExerciseRunner):
    ...
    def run(self, client: redis.Redis) -> dict:
        ...
        self.log.concept("RediSearch indexes hash keys matching a prefix pattern (e.g., product:*).")
        ...
        try:
            ...
            self.log.command("FT.CREATE idx:products ON HASH PREFIX 1 product: SCHEMA ...")
            self.log.success("Index created")
        except redis.exceptions.ResponseError as e:
            self.log.warn(f"FT.CREATE not available: {e}")
            results["search_available"] = False
            self.log.separator()
            self.log.success("RediSearch requires Redis Stack — exercise complete (conceptual)")
            return results
        ...
        products = [
            ("product:1", {"title": "Redis T-Shirt", "description": "Cool cotton shirt with redis logo", "price": "25", "category": "clothing"}),
            ("product:2", {"title": "Redis Hoodie", "description": "Warm hooded sweatshirt for developers", "price": "55", "category": "clothing"}),
            ("product:3", {"title": "Redis Book", "description": "Comprehensive guide to redis internals", "price": "40", "category": "books"}),
        ]
        ...
        results_text = client.execute_command("FT.SEARCH", "idx:products", "shirt | hoodie", "LIMIT", "0", "5")
        ...
        results_price = client.execute_command("FT.SEARCH", "idx:products", "@price:[0 50]", "LIMIT", "0", "5")
        ...
        results_tag = client.execute_command("FT.SEARCH", "idx:products", "@category:{clothing}", "LIMIT", "0", "5")
        ...
        self.log.separator()
        self.log.success(f"RediSearch: text={results['text_search']}, price={results['price_filter']}, tag={results['tag_filter']}")
        return results
