...
class Ex06SortedSets(ExerciseRunner):
    ...
    def run(self, client: redis.Redis) -> list:
        ...
        # ── Step 1: ZADD — build a leaderboard ─────────────────
        ...
        self.log.output(f"Score now: {new_score}")
        ...
        self.log.command(
            f"ZCOUNT {key} 8000 9000 → {mid_tier}, ZCOUNT 9000 +inf → {elite}"
        )
        ...
        self.log.separator()
        self.log.success(
            f"Sorted set ops: size={results[0]}, top={results[1][0][0]}, "
            f"eve={results[4]}→{results[5]}, mid={results[7]}, elite={results[8]}"
        )
        return results
